...
// 
// 		/* swap in 的页使用anonymous 页分配，但是释放时因为有
// 			mapping，使用pagecache页释放，这里有问题。 */
// 		if(mapping)
...
// 		int nr_freed;
// 
//     while (nr_scan++ < PG_RECLAIM_CNTS && !list_empty(&zone->inactive_list)) {
...
// 
// 	sc->nr_to_reclaim = PG_RECLAIM_CNTS;
// 
...
// 			sc->nr_to_scan = min(nr_to_scan_active, PG_RECLAIM_CNTS);
...
// 			sc->nr_to_scan = min(nr_to_scan_inactive, PG_RECLAIM_CNTS);
...
// 
// 		/* 回收数量满足，返回 */
// 		// if(total_reclaimed >= PG_RECLAIM_CNTS) {
...
/**
 * 直接从lru list上回收page，由原来的shrink_list更改。
 */
static int reclaim_pages_from_lru_list() {
	uint32_t reclaimed = 0;
	LIST_HEAD(ret_pages);
	list_head_t *lru_list = &(memory_zone.lru_list);
	
	/* look through lru list*/
	while(!list_empty(lru_list) && reclaimed < PG_RECLAIM_CNTS) {
		address_space_t *mapping;
		page_t *page;
		void (*free_func) (void *addr);

		page = lru_to_page(lru_list);

		/* 从lru链表中取下,取下后清除对应的位，使用del_page_from_lru_list更好？ */
		acquire(&memory_zone.lru_lock);
		list_del(&page->lru);
		if(!TestClearPageLRU(page)) ER();
		release(&memory_zone.lru_lock);
		...
		keep_locked:
			ClearPageLocked(page);
		keep:
			// if(!TestSetPageLRU(page)) ER();
			if(TestSetPageLRU(page)) ER();
			list_add(&page->lru, &ret_pages);
	}

	sfence_vma();
	list_splice(&ret_pages, lru_list);
	return reclaimed;
}

/**
 * 分别从lru链表和pagecache中进行页回收。
 * 注意，一个页可能既在pagecache中，又在lru链表中。
 */
void free_more_memory(void)
{
// 	int u1, u2;
// 
// 	/* 以pagecache为主体进行页回收，主要回收未映射（mmaped）虚拟
// 			地址空间中的页 */
// 	u1 = atomic_get(&used);
// 	reclaim_pages_from_pagecaches();
// 	u2 = atomic_get(&used);
// 	/* 从pagecache中回收了足够的页 */
// 	if(u1 - u2 > PG_RECLAIM_CNTS)
// 		return;

	/* 以lru链表为主体进行页回收，主要回收映射到虚拟地址空间中的页 */
	reclaim_pages_from_lru_list();
}