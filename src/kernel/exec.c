...
static int loadseg(pagetable_t pagetable, uint64 va, FIL *f, uint offset, uint sz);
/**
...
exec(char *path, char **argv){
  ...
  //Check ELF header
  if((r = f_read_off(&f, (void *)&elf, sizeof(elf), &br, 0)) != FR_OK){
    panic("exec2");
    goto bad;
  }
  if(elf.magic != ELF_MAGIC)
...
static int
loadseg(pagetable_t pagetable, uint64 va, FIL *f, uint offset, uint sz)
{
  uint i, n;
  uint64 pa;
